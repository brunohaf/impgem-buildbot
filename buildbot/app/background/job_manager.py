...
from buildbot.app.repository.job.repository import JobRepository
from buildbot.app.repository.job.schemas import Job, JobStatus
...
async def run(runner_job: RunnerJob, job_repository: JobRepository = Depends()) -> str:
    ...
    try:
        ...
        logger.bind(job_id=runner_job.job_id).add(
            log_file,
            level="INFO",
            rotation="10 MB",
            compression="zip",
